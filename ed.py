...
def gen_proc_vid():
  while True:
     ...
     # lines = cv.HoughLinesP(raw1,rho=1,theta=np.pi/180,threshold=100 ,minLineLength=100,maxLineGap=12)
      ...
      #imag = raw1
      frame = og.copy()
      emlt=cv.getStructuringElement(cv.MORPH_RECT,(3,3))
     ...
     # v[:,:]=55
     # og=v
      og = cv.GaussianBlur(og,(13,13),12)
      og = cv.dilate(og,emlt,iterations=3)
      og = cv.GaussianBlur(og,(13,13),5)
      og=cv.cvtColor(og,cv.COLOR_BGR2GRAY)
      og = cv.adaptiveThreshold(og,105,cv.ADAPTIVE_THRESH_GAUSSIAN_C,cv.THRESH_BINARY_INV,15,3)
 ...
 #     lines=[("stupid")]
      if lines is  None:
          x, img_data = cv2.imencode('.jpg', prc)
          raw_bytes = img_data.tobytes()
          yield (b'--frame\r\n'
          b'Content-Type: image/jpeg\r\n\r\n' + raw_bytes +b'\r\n')
          continue  
